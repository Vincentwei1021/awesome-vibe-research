...
信頼とツールチェーンの成熟に伴い、vibe research は自然と 3 つの段階を経ていきます。

| 段階 | 役割 | Agent がやること | 人間が担うこと |
|------|------|-----------------|---------------|
| **Stage 1: インターンとしての Agent** | 実行のみ | 人間が設計した実験を実行する。アイデア提案も研究課題の提示もなし——明確に定義されたタスクに対して信頼性の高いスループットを提供する。 | 問題定義、実験設計、解釈、すべての意思決定。 |
| **Stage 2: 研究者としての Agent** | 一つの研究課題内で独立して活動 | 一つの research question をエンドツーエンドで担う。実験レベルで自己ループ: 実験を提案し、実行し、結果を解釈し、改善する——すべて一つの研究課題の範囲内で完結する。 | 研究課題の設定、課題間の優先順位付け、最終判断。 |
| **Stage 3: 研究リードとしての Agent** | プロジェクト全体の自律的推進 | 研究プロジェクト全体を主導する。研究課題を自ら構築し、仮説を立てて検証し、accept / reject を決定し、研究の方向性を定める。 | 戦略的な監督、倫理審査、出版判断。 |

現在、ほとんどのチームは Stage 1 で運用しています。Vibe research ツールの目標は、Stage 2 を信頼性高く運用可能にし、Stage 3 を実現可能にすることです。
